...
A working exploit and confirmed real-world exploitation are not the same thing. Three of these six had a fully working exploit chain built and published — a Kubernetes pod escape (Netfilter x_tables), a local root chain (AF_PACKET), and demonstrated blind reset/injection attacks (Challenge-ACK) — but as of this writing, only Netfilter x_tables ([CISA KEV](https://www.cisa.gov/known-exploited-vulnerabilities-catalog), added October 2025) has ever been confirmed under active exploitation against real targets, years after its patch shipped. The other demonstrated exploits remain, as far as public records show, research and disclosure artifacts rather than bugs known to have been weaponized in the field — a reminder that "exploitable" and "exploited" carry very different evidentiary bars.
