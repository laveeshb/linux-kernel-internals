...
A 2017 comment correctly reasoned that one reader path was RCU-safe and never checked whether a second one was — nine years later, an OpenAI researcher reported it through normal channels two days before a scheduled exploit competition, unaware a STAR Labs researcher had already independently found and weaponized it as a 0-day for that same competition, using AI to help build the local-root exploit chain.
...
| Found via fuzzing (syzkaller) or automated tooling | No | Partial | No | No | Yes | Yes | No |
