...
This exact hazard was flagged during design review, twelve years before it shipped as a CVE. Ravinandan Arakali's original UFO scatter-gather patch rejected any multi-call `MSG_MORE`/`UDP_CORK` sequence outright — `if (skb_queue_len(&sk->sk_write_queue)) return -EOPNOTSUPP;` — and [David Miller flagged that guard in review](https://lore.kernel.org/netdev/20050603004106.BAB6A7B990@linux.site/): it "breaks NFS, and other things using MSG_MORE and UDP_CORK." Miller laid out the choice explicitly — either teach the UFO append path to handle corked, multi-call sockets properly, or take a different implementation ("the frag_list patch") that didn't have the problem, and said he preferred the cleaner frag_list approach even though the scatter-gather version measured faster. Arakali's team kept the scatter-gather approach for its performance and added multi-call support to it instead of dropping it — which is precisely the code path that, left unaudited for the different failure mode of a UFO/fragmentation *path switch* mid-sequence, produced CVE-2017-1000112. The 2005 thread was about NFS correctness, not security, but it's the moment the multi-call capability this bug depends on was deliberately kept in the design.
...
- [lore.kernel.org: v2 scatter-gather posting](https://lore.kernel.org/netdev/20050603004106.BAB6A7B990@linux.site/) — Miller's June 2005 review flagging that the multi-call `MSG_MORE`/`UDP_CORK` guard broke NFS, the moment the capability this bug later exploited was kept rather than dropped
