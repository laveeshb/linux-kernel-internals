...
**Two independent efforts surfaced the same bug for unrelated reasons.** One was routine security triage (Zeng); the other was a researcher using AI to hunt for competition bugs, who only learned afterward that the same bug had already been reported upstream (Lee). That's not evidence this particular bug was unusually easy to find — but it's a reminder that once routine triage and AI-assisted hunting both make searching a subsystem cheap enough to run casually, a bug that sat unfixed for nine years can still get rediscovered by more than one independent effort in short order.
