...
Under `permissive`, an inferred link only gates the supplier's `sync_state()` callback — it doesn't block a consumer from probing before its supplier has. Under `on`, the link carries `DL_FLAG_AUTOPROBE_CONSUMER`: a consumer whose supplier hasn't probed yet gets deferred automatically via the driver core's existing `-EPROBE_DEFER` retry mechanism, with no driver code needed to request it. This wasn't fw_devlink's first attempt at a default flip, either — a full year earlier, `fw_devlink=permissive` itself had briefly been made the default (`c442a0d18744`, March 21, 2020), silently froze boot on Raspberry Pi 3B and 4 hardware, and was reverted six days later (`18555cb6db23`) before being re-enabled about a month after that, once a specific handling bug was fixed (`00b247557858`). The `on` saga is the same pattern one level up, with a different mechanism (probe blocking rather than sync-state ordering) and a much longer resolution arc.
...
**Flipping a default that changes probe-order semantics platform-wide exposes a long tail of individual "this device never gets a struct device" corners — one hardware platform, one bus type, one legacy compatibility string at a time — not a single bug a review pass can catch in advance.** R-Car's early-initcall power controller, i.MX's AVIC, Samsung Exynos's `interrupt-map` handling, and Broadcom's stale mailbox phandle are four unrelated instances of the same underlying gap between what fw_devlink assumes and what specific firmware descriptions actually guarantee.
