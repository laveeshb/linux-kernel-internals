...
The same bounds-propagation logic came back once more, publicly, six months later. In November 2021, [Starovoitov posted](https://lore.kernel.org/all/20211101222153.78759-1-alexei.starovoitov@gmail.com/) a further fix carrying `Fixes: 10bf4e83167c` — this page's fix, by name. It wasn't a security bug: "Before commit `10bf4e83167c` the umin was incorrectly ignored. The commit `10bf4e83167c` fixed the correctness issue, but pessimized propagation of 64-bit min max into 32-bit min max and corresponding var_off." A companion patch made the equivalent change on the *signed* side, with a commit message explicit that neither one was exploitable: "There is no security bug here. The verifier was too conservative." Getting the unsigned bounds check right in 2021 had cost the verifier some precision on the signed side too — precision this later series gave back once the security fix had shipped and there was time to revisit it.
...
- [lore.kernel.org: bpf: Fix propagation of bounds from 64-bit min/max into 32-bit and var_off](https://lore.kernel.org/all/20211101222153.78759-1-alexei.starovoitov@gmail.com/) — the November 2021 follow-up carrying `Fixes: 10bf4e83167c`, restoring precision this page's fix had cost; explicitly not a security bug
