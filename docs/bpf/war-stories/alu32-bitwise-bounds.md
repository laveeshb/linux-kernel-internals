...
The patch carries `Reviewed-by: John Fastabend` and `Acked-by: Alexei Starovoitov`. There is no `Link:` trailer to a public posting and no public review thread to quote — consistent with a report handled through a private channel and announced on oss-security once fixes shipped, the same pattern seen across the [security subsystem's war stories](../../security/war-stories.md). Anyone looking for a mailing-list design argument about this CVE will not find one.
...
- [CISA: Known Exploited Vulnerabilities Catalog](https://nvd.nist.gov/vuln/detail/CVE-2021-3490) — does not list CVE-2021-3490
