...
The other thing this page documents is a conclusion. After a decade of holding the line that the verifier could be a security boundary against a hostile program author, the BPF maintainers stopped believing it — publicly, on LWN, in 2019 — and the subsystem eventually shipped a knob to turn unprivileged BPF off by default. That is not a CVE, but it is the most consequential entry here, so it gets a page.

Each incident below has its own page: root-cause analysis grounded in the actual patches and the actual verifier traces the maintainers pasted into their commit messages, plus whatever public mailing-list record exists. For one of them — the 2020 jmp32 revert — that record is a full public thread with a competing implementation from Jann Horn in the same series. For the rest, the fixes were handled privately first and announced on oss-security once they shipped, so the pages say that rather than manufacturing a debate.

## Incidents

Ordered reverse chronologically by public disclosure — newest first.
...
Four and a half months earlier, someone had fixed exactly this bug in the signed half of a two-block pattern. The unsigned half sat unchanged, four lines below the fix, in the fix's own diff.
...
| Pattern | Spectre | 32-bit unsigned | Unpriv off | ALU32 bitwise | jmp32 | Sign extension |
|---------|:-------:|:---------------:|:----------:|:-------------:|:-----:|:--------------:|
...
| Published exploit tool exists | No | No | — | Yes | No | Yes |
