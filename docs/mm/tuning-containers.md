...
| `resources.requests.memory` | `memory.min` (via kubelet) | Scheduling guarantee. The kubelet uses this for QoS classification. |
...
- **Guaranteed** (requests == limits): Container gets exactly what it asks for. The kubelet sets `memory.min` equal to the limit, providing hard protection.
- **Burstable** (requests < limits): Container can use more than requested, up to its limit. Gets `memory.min` set to the request value.
...
This prevents a single container from filling the entire zswap pool and forcing other containers' pages to be written to slow disk swap. The per-cgroup zswap limit was introduced in [commit bf9b828972da](https://git.kernel.org/linus/bf9b828972da) ("zswap: memcg accounting").
...
### memory.high Throttling (v4.6, 2016)

**Commit**: [68600f623d69](https://git.kernel.org/linus/68600f623d69) ("mm: memcontrol: default hierarchy support for memory.high") | [LWN](https://lwn.net/Articles/662925/)

Introduced the soft-limit throttling model where processes are slowed down instead of killed, making container memory management much more predictable.
