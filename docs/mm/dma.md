...
The zone boundaries are architecture-specific. Some architectures (like ARM64) have been moving away from `ZONE_DMA` entirely -- see commit [1a8e1cef7603](https://git.kernel.org/linus/1a8e1cef7603) ("arm64: use both ZONE_DMA and ZONE_DMA32") for the nuances on ARM64 where different devices have different lower limits.
...
The SWIOTLB pool size can be configured at boot with `swiotlb=N` (number of 2KB slabs). The implementation lives in [`kernel/dma/swiotlb.c`](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/kernel/dma/swiotlb.c).
