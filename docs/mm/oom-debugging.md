...
```bash
# Get notified when pressure exceeds threshold
# (500ms of stall time within a 2s window)
echo "some 500000 2000000" > /proc/pressure/memory
# Then poll/epoll the fd for notifications
```
...
`systemd-oomd` (systemd 248+) kills processes based on PSI pressure and swap usage before the kernel OOM killer fires. It acts earlier and more predictably.
...
grep -r "^-1000$" /proc/*/oom_score_adj 2>/dev/null | head -20
```
