...
Thomas Gleixner's rewrite, merged for **Linux 4.8** (2016) — confirmed absent in `kernel/time/timer.c` at v4.7, present at v4.8 — kept the same hierarchical shape but made every level the same size (`LVL_SIZE` = 64 slots): the classic wheel's first level alone had 256 slots (`TVR_SIZE`, one jiffy each) with every level above it at 64 (`TVN_SIZE`), so the rewrite specifically shrank that odd-sized first level down to match the rest, trading some timekeeping precision at the higher levels for cascade operations that are now rare rather than routine. At `HZ=1000`, the real current layout (`kernel/time/timer.c`) is:
