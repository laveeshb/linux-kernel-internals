...
Reaching this needs a namespace with more than one ID mapped, which is exactly what `/etc/subuid`, `/etc/subgid` and the setuid `newuidmap`/`newgidmap` helpers from the `uidmap` package hand every desktop user on a stock Ubuntu or Fedora install — Horn's proof of concept used `newuidmap` for the first level, then wrote the six-extent map directly from inside `NS1`, where it already held `CAP_SETUID`. NVD scores the vector `AC:H`, but the difficulty is in *finding* the shape, not in reproducing it.
...
The commit carries `Fixes: 6397fac4915a`, `Cc: stable@vger.kernel.org`, and both `Tested-by:` and `Reviewed-by:` from Eric Biederman, who also applied it. It shipped in the 4.18.19 and 4.19.2 stable releases, announced on November 14, 2018, with distribution updates following ([USN-3832-1](https://usn.ubuntu.com/3832-1/), 3833-1, 3835-1, 3836-1/-2 for Ubuntu).

**On the discussion: there wasn't one, publicly.** This is worth stating plainly rather than papering over. The patch has no `Link:` trailer, and a search of lore.kernel.org turns up no posting of it to LKML, `linux-security-module` or the containers list before it was applied — it went through `security@kernel.org` under embargo and was merged directly. Its only appearances on a public list are Greg Kroah-Hartman's stable-review postings on November 11, 2018 ([`[PATCH 4.18 349/350]`](https://lore.kernel.org/all/20181111221722.721113385@linuxfoundation.org/) and [`[PATCH 4.19 359/361]`](https://lore.kernel.org/all/20181111221702.182204786@linuxfoundation.org/)), neither of which drew a reply on this patch. Public discussion of record consists of Horn's [oss-security post](https://www.openwall.com/lists/oss-security/2018/11/16/1) five days after the stable releases and the Project Zero issue it links to — not a review thread. Anyone reconstructing this bug's history from mailing lists alone will find the 2017 thread that introduced it and nothing about the fix.
...
- [CISA: Known Exploited Vulnerabilities Catalog](https://www.cisa.gov/known-exploited-vulnerabilities-catalog) — does *not* list CVE-2018-18955; no confirmed in-the-wild exploitation
