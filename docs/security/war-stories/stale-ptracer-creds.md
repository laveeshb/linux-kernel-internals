...
The bug had been present since Linux **4.10** (2016), when [`64b875f7ac8a`](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=64b875f7ac8a5d60a4e191479299e931ee949b67) introduced `ptracer_cred` — over three years before Jann Horn's report. Distributions shipped it embargoed to their security teams and coordinated a release: Debian, Ubuntu, Red Hat, SUSE, NetApp, and F5 all published advisories the same week (see External references), and Ubuntu's own advisory numbers alone (USN-4093-1 through USN-4118-1) span five separate kernel flavors patched in lockstep.
...
The commit carries `Fixes: 64b875f7ac8a` and `Acked-by: Oleg Nesterov <oleg@redhat.com>` — Nesterov being the ptrace subsystem's other longtime reviewer — but **no `Link:` trailer**, and no corresponding public LKML/lore.kernel.org review thread exists for this patch. It went through `security@kernel.org` under embargo and was merged directly, with the patch and Jann Horn's Project Zero writeup going public together on July 17, 2019, rather than being reviewed on a development list first.

The fix landed in mainline for 5.2 and was backported within a week to 5.1.17, 4.19.58, 4.14.133, 4.9.185, 4.4.185, and later 3.16.71 — a wide stable-tree spread reflecting how long-lived the bug was (present since 4.10, but the underlying `ptrace_link()` split predates that).
...
- [Exploit-DB 47133](https://www.exploit-db.com/exploits/47133), [47163](https://www.exploit-db.com/exploits/47163), [50541](https://www.exploit-db.com/exploits/50541) — Horn's original report and PoC (47133), plus two independent reimplementations: bcoles (47163, 2019-07-24) and Ujas Dhami (50541, 2021-11-23)
