...
The bug had been present since Linux **4.10** (2016), when [`64b875f7ac8a`](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=64b875f7ac8a5d60a4e191479299e931ee949b67) introduced `ptracer_cred` — over three years before Jann Horn's report. Distributions shipped it embargoed to their security teams and coordinated a release: Debian, Ubuntu, Red Hat, SUSE, NetApp, and F5 all published advisories the same week (see External references), and Ubuntu's own advisory num­bers alone (USN-4093-1 through USN-4118-1) span five separate kernel flavors patched in lockstep.
...
The commit that introduced `ptracer_cred`, `64b875f7ac8a`, was itself a security fix — it replaced the coarser `PT_PTRACE_CAP` bit specifically because that bit didn't distinguish *which* user namespace the tracer was capable in. Eric Biederman's message for that 2016 commit already flagged that the `PTRACE_TRACEME` path had been "overlooked" once before when `PT_PTRACE_CAP` was first added; the same call site was overlooked a second time when the bit was replaced with a full credential pointer, in the same way: correct in the common case (`PTRACE_ATTACH`), silently wrong in the other. There's also a secondary, smaller bug the fix folds in: taking an RCU reference to `new_parent`'s credentials and then calling `get_cred()` on it after `rcu_read_unlock()` isn't a valid way to obtain a stable reference under `struct cred`'s object-lifetime rules — a second reason the old code path was unsound independent of whose credentials it was even reading.
...
The commit carries `Fixes: 64b875f7ac8a` and `Acked-by: Oleg Nesterov <oleg@redhat.com>` — Nesterov being the ptrace subsystem's other longtime reviewer — but **no `Link:` trailer**, and no corresponding public LKML/lore.kernel.org review thread was findable for this patch (lore's search interface bot-gates automated fetches, and no oss-security post references it either — Red Hat's Tomer Brisker posted an unrelated CVE to oss-security the same week, but CVE-2019-13272 itself has no entry there). That's consistent with how the kernel security team has historically handled a subset of embargoed local-privilege-escalation fixes: reviewed privately with the security list and named ack-ers before the patch and its Project Zero writeup went public together on July 17, 2019, rather than being publicly reviewed on a development list first. Where this page can't point to a review thread, it says so rather than inventing one.

The fix landed in mainline for 5.2 and was backported same-day to 5.1.17, 4.19.58, 4.14.133, 4.9.185, 4.4.185, and later 3.16.71 — a wide stable-tree spread reflecting how long-lived the bug was (present since 4.10, but the underlying `ptrace_link()` split predates that).
...
**"Overlooked once" is a signal to audit, not a coincidence.** The commit that introduced `ptracer_cred` explicitly noted that `PTRACE_TRACEME` had already been missed once, when `PT_PTRACE_CAP` was first added. The same call site was missed again three years later during the credential-pointer rewrite — a reminder that a code comment flagging a past miss is a to-do, not a closed loop.
...
- [Exploit-DB 47133](https://www.exploit-db.com/exploits/47133), [47163](https://www.exploit-db.com/exploits/47163), [50541](https://www.exploit-db.com/exploits/50541) — three independently published local-root proof-of-concept exploits
...
- [Debian DSA-4484](https://www.debian.org/security/2019/dsa-4484), [Ubuntu USN-4093-1](https://usn.ubuntu.com/4093-1/) (and USN-4094-1, 4095-1, 4117-1, 4118-1), [Red Hat RHSA-2019:2405](https://access.redhat.com/errata/RHSA-2019:2405) — representative downstream vendor advisories from the coordinated July 2019 release