...
> CVE-2019-13272 — `PTRACE_TRACEME` recorded the *parent's* credentials as the ptracer's, so an unprivileged child could make its still-privileged parent its ptracer, then wait for that parent to drop privileges and become attacker-controlled while the relationship stayed root-marked
