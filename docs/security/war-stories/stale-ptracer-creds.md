...
sysctl -w kernel.yama.ptrace_scope=3   # 0/1 do not stop this bug (1 = Ubuntu/Debian default,
                                        ...
                                        # until reboot; lower values can still be lowered again.
```
...
**"Overlooked once" is a signal to audit, not a coincidence.** The commit that introduced `ptracer_cred` explicitly noted that `PTRACE_TRACEME` had already been missed once, when `PT_PTRACE_CAP` was first added. The same call site was missed again fifteen years later, in the 2016 credential-pointer rewrite itself — a reminder that a commit message flagging a past miss is a to-do, not a closed loop.
