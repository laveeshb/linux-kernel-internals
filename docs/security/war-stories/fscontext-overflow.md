...
Linux 5.1 shipped the new mount API — `fsopen()`, `fsconfig()`, `fsmount()`, `move_mount()` — built on David Howells' filesystem context work, merged as [`3e1aeb00e6d1`](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=3e1aeb00e6d132efc151dacc062b38269bc9eccc) ("vfs: Implement a filesystem superblock creation/configuration context"). Instead of handing the kernel one opaque option string at `mount()` time, userspace now opens a *context* and feeds it parameters one at a time, each as a separate syscall.
...
The `- 2` reserves room for the separating comma, the `=`, and the trailing NUL. `size` is an `unsigned int`; `PAGE_SIZE` is an unsigned long. There is no signed value anywhere in that expression.
...
On the next call, `PAGE_SIZE - 2 - size` evaluates to `4096 - 2 - 4095`. In unsigned arithmetic that is not `-1`; it is `ULONG_MAX`. No `len` can exceed it, the guard never fires, and `memcpy()` writes at `ctx->legacy_data + 4095` and beyond. Because `size` keeps growing, every subsequent `fsconfig()` call writes further past the end — a repeatable, attacker-positioned, attacker-contented heap write, bounded per call only by `fs/fsopen.c`'s `strndup_user(_value, 256)` limit on key and value length.
...
Reaching `fsopen()` at all requires `CAP_SYS_ADMIN` — but only in the caller's *current* user namespace. As the [oss-security announcement](https://www.openwall.com/lists/oss-security/2022/01/18/7) put it: "An unprivileged user can use `unshare(CLONE_NEWNS|CLONE_NEWUSER)` to enter a namespace with the CAP_SYS_ADMIN permission, and then proceed with exploitation to root the system." On any distribution shipping unprivileged user namespaces — Ubuntu, Debian, and every container runtime that relies on them — the practical privilege requirement is none, which is why NVD scores this `PR:N` despite it being a local bug.

## Observed behavior

The kernel-visible symptom is a KASAN slab-out-of-bounds write in the `kmalloc-4k` cache, reported by both fuzzers that found it:
...
They used it to escape Google's hardened kCTF Kubernetes containers and read another pod's flag, earning a $31,337 bounty — reduced from the maximum only because of a collision with an earlier, unreported discovery by StarLabs. A week later, Alejandro Guerrero [published an independent exploit](https://www.openwall.com/lists/oss-security/2022/01/25/14) on oss-security targeting Ubuntu 21.04 with kernel 5.11. Two and a half years after that, CISA added CVE-2022-0185 to its [Known Exploited Vulnerabilities catalog](https://www.cisa.gov/known-exploited-vulnerabilities-catalog) (21 August 2024), and NVD's SSVC data records its exploitation status as **active**. There is no Exploit-DB entry and no Metasploit module; the [Crusaders of Rust repository](https://github.com/Crusaders-of-Rust/CVE-2022-0185), published 19 January 2022, is the exploit distribution channel, and NVD tags it as such.
...
The trailers are the interesting part. The patch carries `Signed-off-by` from Hill-Daniel and William Liu, `Tested-by` from Salvatore Bonaccorso (Debian) and Thadeu Lima de Souza Cascardo (Canonical), and `Acked-by` from both Dan Carpenter and Al Viro, before Greg Kroah-Hartman's and Linus Torvalds' sign-offs. That is a distribution-security review chain, not a public mailing-list one — and it shows: **there is no lore.kernel.org thread for this patch.** The commit has no `Link:` trailer, and no posting of it to linux-fsdevel or LKML exists in the archives. The bug was reported privately (Red Hat's [Bugzilla 2040358](https://bugzilla.redhat.com/show_bug.cgi?id=2040358) was filed on 13 January 2022), reviewed under embargo, and pushed straight to mainline on the day of public disclosure. The public discussion happened elsewhere: the [oss-security announcement](https://www.openwall.com/lists/oss-security/2022/01/18/7) on 18 January, John Haxby's reply the same hour assigning CVE-2022-0185, and Guerrero's exploit post a week later. Anyone reconstructing this fix's history from lore alone will find only the four stable-tree backport commits.
...
**A new syscall can weaponize old code without changing a line of it.** `legacy_parse_param()` was as correct in 5.0 as in 5.1; what changed was that `fsconfig()` let an attacker drive it 35 bytes at a time instead of handing it one string. When you add a finer-grained interface to an existing subsystem, the compatibility shim behind it is now reachable in ways its author never modelled.
