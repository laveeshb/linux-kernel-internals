...
"Nobody noticing" isn't quite right, either — someone looked at this exact function's arithmetic and fixed the line right next to the bug, three and a half years before the CVE. In August 2018, while `legacy_parse_param()` was still under development on David Howells' branch, Andrei Vagin [posted a one-line fix](https://lore.kernel.org/linux-fsdevel/20180802072810.3126-1-avagin@openvz.org/) to the very next statement in the same function: `size += len;` should have been `size += param->size;`, since `len` didn't yet account for a string value's actual length. Howells folded it in the same day. That patch touched the accumulator the underflow depends on and still missed the underflow — a second illustration of the same lesson this bug already teaches: reviewing one line of a length calculation correctly says nothing about whether the check bounding it is sound.
