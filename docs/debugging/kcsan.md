...
KCSAN uses hardware **watchpoints** to detect racing accesses:

```
Thread 1 write to &x:
  1. KCSAN randomly decides to set a watchpoint on &x
  2. Sets hardware debug register (or software simulation) to trap on &x access
...
CONFIG_KCSAN_WATCHPOINT_COUNT=64        # hardware watchpoints per CPU
...
# Build:
make KCSAN=1 ...
# or: set CONFIG_KCSAN=y in .config
```
