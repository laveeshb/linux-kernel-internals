...
A researcher reported on oss-security that `flush_to_ldisc()` could compute a **negative** byte count — `smp_load_acquire(&head->commit) - head->read` going negative when the racing insert and read observed inconsistent state — leading to an out-of-bounds read and disclosure of kernel slab memory. Per Jiri Slaby's fix commit and NVD's own CVE description, the race was reproduced by racing `pty_write()` against `ioctl(TIOCSTI)` and `ioctl(TCXONC)` on the other end of the pair — both of which also insert characters into the same tty buffer outside of `pty_write()`'s call path. This is CVE-2022-1462, rated CVSS 3.1 6.3 (MEDIUM).
