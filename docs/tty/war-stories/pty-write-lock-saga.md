...
Holding `port->lock` across `tty_flip_buffer_push()` turned out to have a cost the 2018 fix didn't anticipate: `tty_flip_buffer_push()` can, through the console/printk path, end up needing locks (`console_owner`, workqueue `pool->lock`) that were also reachable while holding `port->lock` from other code paths — lockdep flagged a genuine circular lock-ordering dependency, a real deadlock risk under the right interleaving.

Commit `71a174b39f10b4b93223d374722aa894b5d8a82e` ("pty: do tty_flip_buffer_push without port->lock in pty_write", Artem Savkov, September 2020) resolves the lockdep cycle by moving `tty_flip_buffer_push()` back **outside** `port->lock` — but the insert itself stays locked. This looks like a narrower, more surgical fix than reverting Act 1 entirely. It isn't: `tty_flip_buffer_push()` reads the buffer's `commit` field (via `smp_load_acquire()`) to know how much data is ready to hand to the line discipline, and once that read moves outside the lock, a concurrent insert can leave `commit` in an inconsistent state relative to what the unlocked reader observes — the 2018 race, reborn in the commit-tracking field instead of the raw insert.

## Act 3 (2022): the reborn race gets a CVE

A researcher reported on oss-security that `flush_to_ldisc()` could compute a **negative** byte count — `smp_load_acquire(&head->commit) - head->read` going negative when the racing insert and read observed inconsistent state — leading to an out-of-bounds read and disclosure of kernel slab memory. This is CVE-2022-1462, rated CVSS 3.1 6.3 (MEDIUM).
