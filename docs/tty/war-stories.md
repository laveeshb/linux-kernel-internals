...
`n_hdlc.tbuf`, a single unsynchronized pointer tracking "the buffer currently being retransmitted after a tx error," predates the bug in the HDLC line discipline (`drivers/tty/n_hdlc.c`, loadable via `N_HDLC`). A 2009 commit added the `flush_tx_queue()` path (triggered by a `TCFLSH` ioctl or ldisc close), which raced against that pre-existing `tbuf` pointer: concurrent `flush_tx_queue()` and `n_hdlc_send_frames()` (transmit completion) could both see the same buffer through `tbuf` and both queue it onto the free-buffer list — a double-free, reachable by a local unprivileged user, that researcher Alexander Popov turned into a working root exploit bypassing SMEP.
