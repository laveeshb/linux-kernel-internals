...
[Command Submission, `dma_fence`, and the GPU Scheduler](../command-submission.md) documents the cross-driver `dma_fence` signaling contract: a fence must be signalable without depending on any lock or resource that an allocation elsewhere in the kernel might be blocked on, because `dma_fence` waits are load-bearing all the way down into memory reclaim — the page allocator itself can end up waiting on a GPU fence to make forward progress. The rule, in the contract's own words (quoted on that page): code on the signaling path must not allocate memory that could recurse into reclaim.
...
The crash-dump allocation was ordinary code, written without the `dma_fence` signaling-path constraint in mind, because on its face `msm_gpu_crashstate_capture()` has nothing to do with fences — it's diagnostic tooling, not the fast path. The violation only exists because of what that allocation could transitively trigger under memory pressure: reclaim, into the GPU's own shrinker, into a fence wait that the allocating thread's own held lock was blocking. Nothing about the allocation call site looks unsafe in isolation; the danger is entirely in the reachable call graph under load, which is exactly the kind of hazard the cross-driver `dma_fence` contract exists to name explicitly rather than leave to be rediscovered per driver.
...
**The `dma_fence` signaling-path rule isn't about the code that waits on fences — it's about everything reachable from code that could block a fence signal.** `msm_gpu_crashstate_capture()` never touches a `dma_fence` directly. It became a signaling-path violation transitively, through an allocation that could recurse into the driver's own shrinker.
