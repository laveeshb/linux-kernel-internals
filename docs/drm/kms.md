...
`mode_changed` is the flag the atomic core checks against `DRM_MODE_ATOMIC_ALLOW_MODESET`, covered below.
...
- **`drm_atomic_check_only(struct drm_atomic_commit *state)`** (`drivers/gpu/drm/drm_atomic.c`) — validates the proposed state against `config->funcs->atomic_check`, and additionally rejects the commit with `-EINVAL` if any CRTC's `mode_changed` is set but `state->allow_modeset` is false. No hardware is touched. Drivers implementing `atomic_check` typically call the generic **`drm_atomic_helper_check(struct drm_device *dev, struct drm_atomic_commit *state)`** (`include/drm/drm_atomic_helper.h`) as their first step, which runs the shared plane/CRTC/connector cross-validation (does every enabled CRTC have a mode and at least one connector, do plane geometries fit, etc.) before any hardware-specific checks.
...
- **`DRM_MODE_ATOMIC_ALLOW_MODESET`** — without it, any change that would require a real modeset (`crtc_state->mode_changed`) is rejected. Compositors set this only for actual mode changes and leave it off for routine per-frame plane/property updates, so a buggy commit can't accidentally force a full, visible modeset.
...
- [LWN: "prepare for atomic/nuclear modeset/pageflip"](https://lwn.net/Articles/600369/) — Rob Clark's May 2014 infrastructure RFC that started the transition
