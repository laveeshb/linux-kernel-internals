...
- **`dma_buf` and `import_attach` are the two dma-buf linkage points**, and they're mutually telling: a GEM object that's been *exported* has `dma_buf` set (pointing at the dma-buf wrapping it); one that's been *imported* has `import_attach` set instead (pointing at this device's attachment to someone else's dma-buf). `drm_gem_is_imported()` is just `!!obj->import_attach`.
...
A GEM object needs some integer userspace can pass through an ioctl — you can't hand a raw kernel pointer across the syscall boundary. DRM has had three answers to "which integer," introduced in roughly this order, each fixing a security problem in the last:
...
struct ttm_buffer_object {
	...
	struct ttm_tt *ttm;              // page table for the "TT" (system-memory) case
	...
};
```
...
**3. Mapping for actual GPU access.** Before the GPU can read the frame's pixels, something along this path calls `dma_buf_map_attachment()` for the GPU's attachment, getting back an `sg_table` DMA-mapped for the GPU's own address space/IOMMU domain — a V4L2 capture engine and a GPU render engine can easily have different reachable physical ranges, which is exactly the case the attach/map split exists for.

**4. Fencing keeps it safe.** Whatever fence the V4L2 driver attached to the buffer's (now shared) `dma_resv` for the capture DMA itself — a `DMA_RESV_USAGE_WRITE` fence marking "capture into this buffer is still in flight" — is visible to the GPU driver through that same shared `resv`. The GPU's texture-sampling job either waits on that fence explicitly, or (for implicit-sync submission models) has it folded automatically into the dependencies the kernel inserts before the job runs, so the texturing hardware never starts reading pixels the capture engine hasn't finished writing.

**5. Teardown.** The compositor eventually calls `DRM_IOCTL_GEM_CLOSE` on its GPU-side handle. Handle release, not the dma-buf fd itself, is what drops the GPU driver's GEM object reference; the underlying dma-buf (and hence the V4L2 buffer it wraps) stays alive as long as anyone else — including the V4L2 driver's own queue — still holds a reference to it.

The same shape — export on one driver's GEM handle via `PRIME_HANDLE_TO_FD`, import on another driver's DRM fd via `PRIME_FD_TO_HANDLE`, `dma_resv`-mediated fencing throughout — is exactly how a render GPU hands a composited frame to a physically separate display-output GPU on a hybrid-graphics laptop; only which side is "exporter" and which is "importer" changes.
...
- [`3248877ea179`](https://github.com/torvalds/linux/commit/3248877ea1796915419fba7c89315fdbf00cb56a) — "drm: base prime/dma-buf support (v5)," Dave Airlie, November 2011: DRM's GEM-handle-to-dma-buf-fd binding
