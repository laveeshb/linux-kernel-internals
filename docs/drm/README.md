...
Before DRM, X servers (XFree86) drove graphics hardware directly from userspace, running as root and poking hardware registers itself — a single privileged, crash-prone process, internally duplicating root-level hardware access across its own per-chipset drivers. A bug there could hang or corrupt the whole machine, and there was no kernel arbiter to stop two clients from fighting over the same display or GPU memory. DRM arrived in 1999 to arbitrate *rendering* among multiple clients, but modesetting stayed in X's hands for almost another decade — it only moved into the kernel with **KMS** in 2008, and X didn't fully give up running as root until DRM-Master handover matured years after that.
