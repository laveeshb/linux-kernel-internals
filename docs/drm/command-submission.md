...
`amdgpu_job_alloc()` bottoms out in `drm_sched_job_init()`, which is where a driver-private submission becomes a scheduler-visible object. That is the definition of a job worth holding on to: **a job is one unit of work that the scheduler will hand to the driver's `run_job()` callback exactly once, that carries its own list of dependency fences, and whose completion is represented by a fence other code can wait on.** Everything hardware-specific stays in the driver's containing struct (amdgpu's `struct amdgpu_job` embeds `struct drm_sched_job base` as its first member, the same embedding pattern GEM uses for buffer objects).
...
3. **`drm_sched_job_arm(job)`** — picks the scheduler (`drm_sched_entity_select_rq()`), stamps `job->sched` and `job->s_priority`, and initializes the two fences with real sequence numbers. Its kerneldoc calls this *"a point of no return"*: once armed, the job **must** be pushed, because other code may now be holding its `finished` fence.
...
1. **`drm_sched_select_entity()`** walks run-queues from `DRM_SCHED_PRIORITY_KERNEL` downward and returns the first ready entity. Crucially, if a run-queue has a ready entity but the scheduler is out of credits, the selector returns `ERR_PTR(-ENOSPC)` rather than `NULL`, and the loop stops — so a busy ring does not let low-priority work jump ahead of blocked high-priority work.
