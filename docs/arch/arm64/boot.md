...
SYM_CODE_START(primary_entry)
    ...
    /*
     * Determine CPU endianness and set LE mode.
     * ARM64 Linux always runs in little-endian.
     */
    adr_l   x0, .Lpi
    msr     SCTLR_EL1, x0
    isb
...
SYM_FUNC_START_LOCAL(__create_page_tables)
    ...
    adrp    x0, init_pg_dir
    adrp    x6, _text
    bl      __map_kernel            /* maps _text.._end at kernel VA */
    ret
...
SYM_FUNC_START_LOCAL(__primary_switched)
    ...
    add     sp, x5, #THREAD_SIZE    /* kernel stack top */

    /* Initialize the thread pointer (TPIDR_EL1 = current) */
