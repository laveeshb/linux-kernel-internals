...
Setting `PR_SET_CHILD_SUBREAPER` doesn't affect this, but `prctl(PR_SET_TIMERSLACK)` and some container runtimes use `sched_reset_on_fork` to prevent privilege escalation via fork:
...
Historically, Linux ran the parent first (to avoid the COW cost if the child execs immediately). This policy has been revisited multiple times. The current behavior depends on `sysctl kernel.sched_child_runs_first`:

```bash
# 0 (default): parent may run first (scheduler decides)
# 1: hint to run child first
cat /proc/sys/kernel/sched_child_runs_first
```

When set to 1, `wake_up_new_task()` calls `set_next_buddy(&p->se)` which hints to EEVDF to prefer the child.
